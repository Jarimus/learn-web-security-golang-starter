...
func (store *Store) VerifyAndConsume(ctx context.Context, userID int64, code, secret string) (bool, error) {
	...
	dbResult, err := store.queries.ConsumeTOTPStep(ctx, dbgen.ConsumeTOTPStepParams{
		TimeStep: &step,
		UserID:   userID,
	})
	if err != nil {
	...
	if changedRows, err := dbResult.RowsAffected(); err != nil || changedRows != 1 {
...
func (store *Store) ConsumeBackupCode(ctx context.Context, userID int64, code string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("consume TOTP backup code query: %w", err)
	}
	count, err := result.RowsAffected()
	if err != nil {
		return false, fmt.Errorf("consume TOTOP backup code rows affected: %w", err)
	}
	return count == 1, nil
}
