...
func DecryptShippingDetails(serialized string, keyring *storage.Keyring) (ShippingDetails, error) {
	serializedDecrypted, err := keyring.Decrypt(serialized)
	if err != nil {
		return ShippingDetails{}, fmt.Errorf("decrypt serialized shipping details: %w", err)
	}
	var details serializedShippingDetails
	if err := json.Unmarshal(serializedDecrypted, &details); err != nil || details.Name == nil || details.Address == nil || details.City == nil || details.Region == nil || details.PostalCode == nil {
